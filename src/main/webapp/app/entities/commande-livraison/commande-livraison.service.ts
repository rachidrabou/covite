...
export class CommandeLivraisonService {
  ...
  create(commandeLivraison: ICommandeLivraison): Observable<EntityResponseType> {
    return this.http.post<ICommandeLivraison>(this.resourceUrl, commandeLivraison, { observe: 'response' });
  }

  update(commandeLivraison: ICommandeLivraison): Observable<EntityResponseType> {
    return this.http.put<ICommandeLivraison>(this.resourceUrl, commandeLivraison, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ICommandeLivraison>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<ICommandeLivraison[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
